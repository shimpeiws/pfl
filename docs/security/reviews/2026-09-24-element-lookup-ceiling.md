...
| **Resource ceilings**                      | The lookup is bounded by `MAX_ELEMENT_LOOKUP_RUNS` (≤ 20 observation re-reads) on top of a full `listRuns` scan — the same walk `pfl snapshots`, `gc`, and every named `--snapshot` read already perform, so the strategy is the store's accepted one rather than a new unbounded path. |
...
| 3   | Info     | `listRuns` parses every stored artifact before the 20-run candidate cap applies, so a large store makes one failed `show` scan the entire history.                  | **Accepted.** The scan is the resource strategy the store already runs for `snapshots`, `gc`, and named reads; `gc` keeps run counts small and the cap still bounds the second pass. The code comment now states this explicitly instead of claiming a bounded store walk. |
| 4   | Bug      | A located run whose resolved artifact is unreadable (`resolvedId: null`) was suggested as a `--snapshot` target, but the suggested command cannot show the element. | **Fixed.** `foundInOtherRunError` reports that the element is in that run but cannot be shown until a resolved snapshot exists; only a run with a readable resolved snapshot earns the `--snapshot` hint.                                                                  |
