...
| 3   | `detect-non-literal-regexp` in `src/redact/output.ts`: `new RegExp` built from a value | **Accepted as a false positive, annotated.** The pattern is built from `escapeRegExp(home)` (a trusted, escaped local value) plus a lookahead; a `// nosemgrep` comment records the reasoning |
