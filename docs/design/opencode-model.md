...
observations came from an earlier version of it (`e98dbfc`); the load-bearing
details below and the **1.18.31 only** marks in the exercised list delimit where
the two differ. That earlier version's same-name element markers
(`agent/command/skill/marker`) are identical to the ones above, but its config
...
- **`git init`, not `mkdir .git`.** §2's walk stop only appears against a real
  ...
  parent directory's config — a different input shape, not the opposite output for
  the same one, and the reason `git init` is what the probe uses.
...
`tool(s)/`, `theme/`, `formatter`, `lsp`, the `instructions` array, and the
`provider`/`disabled_providers`/`enabled_providers` keys; plugins
...
measured on the §0 probe on 1.18.31, and **all four are 1.18.31 only**: they come
...
| Model / provider       | `model`, `small_model`, `provider`, `disabled_providers`, `enabled_providers`    | [installed: measured] (1.18.31 only: `model`, `small_model`; `provider` keys [upstream]) |
...
`debug skill` lists one entry per name, so that listing reflects the collision
winner (§6), which is not stable. This is an absence of evidence about 1.18.30,
not evidence of a version difference. What 1.18.31 does establish is that
`.opencode/skill/<name>/SKILL.md` is **scanned**; only which copy survives a
...
| `runtime-defined` | built-in tool/agent defaults; the compiled-in agents are listed by `agent list` (§0 step 9), which does **not** mark an entry as built in — on 1.18.31 the same output lists the fixture's own agent, while on 1.18.30 the fixture produced no entry at all (§0) | [installed: measured] for the listing; built-in membership and the defaults themselves are [installed: self-described] |
...
of `global` is a single observation the re-probe neither confirmed nor refuted:
`global` recurred (9 of 24 invocations, and the first invocation of four of the
eight runs), but no winner is stable, so one observation could not have established
it as a property of that version. Nor does `global` mark a version difference in
the other direction — the 1.18.31 runs above returned `global` for the first
invocation of four of the eight.
...
`memory` has no surface in the config schema [upstream] or in the built-in skill
body [installed: self-described]; no probe searched for one, so this is not a
claim that OpenCode has none.
...
- **Built-in agents** compiled into the binary: `build`, `plan`, `general`,
  ...
  while on 1.18.30 the fixture produced no entry at all (§0) — so the compiled-in
  claim rests on there being no on-disk definition, not on the listing.
...
- **No 1.18.30 record of the claim as stated.** The user-scope-only elements
  ...
  the walk stop, and the walk-independence check (§2) have no 1.18.30 observation.
- **A record whose fixture or method cannot support the claim.** The precedence
  ladder (the 1.18.30 fixture gave every source the same single key, so it could
  not separate a source that loaded and lost from one that was never read) and the
  MCP resolved key set (the 1.18.30 record counted `mcp` mentions with `grep -ci`,
  not the resolved key set) — both §0.
- **A re-measurement whose outcome is non-deterministic, so the single 1.18.30
  ...
  recurred on 1.18.31 (9 of 24 invocations), so nothing here marks a version
  difference.
...
| §4.1/§7 `mode(s)/` | legacy; M9 records `unsupported` | **Corrected: `mode/<name>.md` loads as a primary agent in both scopes.** The resolved `agent` map lists it with `"mode": "primary"` — measured for a project `.opencode/mode/` file and, on a second isolated probe, for a `~/.config/opencode/mode/` file. Adapter aligned (#149) |
